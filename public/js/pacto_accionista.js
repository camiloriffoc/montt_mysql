//Funcion para agregar por AJAX un nuevo pacto de accionista
	$('.add-pacto-accionista-ajax').on("submit", function(ev){
        ev.preventDefault();
        var $form = $(this);
        ...
        $.ajax({
            ...
            beforeSend: function(){
            	$('#myModalAddPactoAccionista').modal('hide');
                $button.val("Cargando...");
            },
    ...
    $('.update-pacto-accionista-ajax').on("submit", function(ev){
    ...
    $('.delete-pacto-accionista').on('click',function(){
    	var $row = $(this).parents('tr');
    	var $id = $row.data('id');
    	var $form = $('#form-delete-pacto-accionista');
    	var $url = $form.attr('action').replace(':PACTO_ID',$id);
    	var $data = $('#form-delete-pacto-accionista');
        var $formData = new FormData($data);
